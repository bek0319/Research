...
best_model.add(Conv2D(cnn_filter1, (3, 3), activation='relu', input_shape=(X_train.shape[1], X_train.shape[2], 1), padding='same'))
best_model.add(MaxPooling2D((2, 2)))
best_model.add(BatchNormalization())
best_model.add(Conv2D(cnn_filter2, (3, 3), activation='relu', padding='same'))
best_model.add(MaxPooling2D((2, 2)))
best_model.add(BatchNormalization())
best_model.add(Conv2D(cnn_filter3, (3, 3), activation='relu', padding='same'))
best_model.add(MaxPooling2D((2, 2)))
best_model.add(BatchNormalization())
best_model.add(Conv2D(cnn_filter4, (3, 3), activation='relu', padding='same'))
best_model.add(MaxPooling2D((2, 1)))
best_model.add(BatchNormalization())
best_model.add(Conv2D(cnn_filter5, (3, 3), activation='relu', padding='same'))
best_model.add(MaxPooling2D((2, 1)))
best_model.add(BatchNormalization())
best_model.add(Flatten())
best_model.add(RepeatVector(1))
# best_model.add(LSTM(best_params['lstm1'], activation='relu', return_sequences=True))
best_model.add(LSTM(lstm2, activation='relu', return_sequences=True))
best_model.add(LSTM(lstm3, activation='relu', return_sequences=False))
